...
```mermaid
flowchart TD
  A[Discord Gateway\nmessageCreate] --> B{Author is bot?}
  B -- yes --> C[Capture bot message\n+ attachments to memory\n(optional archive to disk)]
  C --> C2[maybeSummarize buffer]
  B -- no --> D[Compute convoKey]
  D --> E[Push user message to Redis/memory]
  E --> I[Addressing rules]
  I{DM / mentioned /\nwake-word?} -- no --> J[Ignore]
  I -- yes --> K[Build context with SYSTEM_PROMPT]
  K --> L[generateText(prompt)]
  L --> M[extractResponseContent + splitByToolCalls]
  M --> N{TTS mode?}
  N -- audio-off --> O[Send text:\n before (+calls pretty) + after]
  N -- audio-only --> P[Generate WAV→MP3,\nSend MP3 only]
  N -- audio + text --> Q[Generate WAV→MP3,\nSend text + MP3]
  O --> R{Calls length > 0?}
  Q --> R
  P --> S[Stop (audio only)]
  R -- no --> T[If no text sent,\nsend raw fallback]
  R -- yes --> U[for each call:\ntryHandleToolCall(line)]
  U --> V[If VERBOSE, send “tool done” + after]
  V --> W[Bot-authored capture\n(assistant) -> memory]
  T --> W
  S --> W
  W --> X[maybeSummarize buffer]

  subgraph Slash commands
    A2[Discord Gateway\ninteractionCreate] --> B2{isCommand?}
    B2 -- no --> C2b[Ignore]
    B2 -- yes --> D2[handlers[command].execute()]
    D2 --> E2[Log to Postgres]
  end
```
